...
global vars
    ...
    ;;; reduced 16 Nov 2003, as CPUs are now much faster than
    ;;;     when this was chosen
    dog_default_speed = 10,
    ;
...
define :class trial_obstacle; is trial_agent;
    ...
    slot rc_pic_lines ==            ;;; What the obstacle looks like
          [
            WIDTH 5 COLOUR 'blue'
            [CIRCLE {0 0 9}]       ;;; Circle with radius = trial_size
          ];
enddefine;

define :class trial_tree; is trial_obstacle;
...
define :class trial_post; is immobile trial_obstacle ;
...
define approach_speed(dist) -> newspeed;
    ;;; Modified 16 Nov 2003 to reduce speed
    ;;; Added by A.S. 19 Jul 2000
    ;;; Speed at which dog should approach sheep
    ;;; reduce speed when getting close.
    if dist > 90 then dog_default_speed else round(dist/15) endif -> newspeed;
enddefine;
...
define :method rc_move_to(agent:trial_agent, x, y, boole);
    dlocal rc_in_event_handler;
    if rc_in_event_handler then

        ;;; run it later.
        rc_defer_apply(rc_move_to(%agent, x, y, boole%))
        ...
        call_next_method(agent, x, y, boole);

    endif;
enddefine;

define :method rc_set_axis(agent:trial_agent, heading, boole);
    dlocal rc_in_event_handler;
    if rc_in_event_handler then

        ;;; run it later.
        rc_defer_apply(rc_set_axis(%agent, heading, boole%))
        ...
        true -> rc_in_event_handler;
        call_next_method(agent, heading, boole);
...
define :method in_pen(agent:trial_dog) -> boole;
    ...
    if sim_distance(agent, agent.trial_goal ) < 55 then
...
define :method current_in_front(agent:trial_dog)->boole;
    ...
    unless agent.trial_leftpost == [] or agent.trial_rightpost == [] then
    rel_direction(trial_coords(agent.selected_sheep), trial_coords(agent.trial_leftpost), trial_coords(agent.trial_goal)) > rel_direction(trial_coords(agent.selected_sheep), trial_coords(agent.trial_rightpost), trial_coords(agent.trial_goal))->boole;
    endunless;
...
define :method quoi(agent:trial_dog) -> boole;
    ...
    rel_direction (trial_coords(agent), trial_coords(agent.selected_sheep), trial_coords(agent)) > c
        -> boole;
enddefine;
...
define :method wise(agent:trial_dog);
    ...
    (ang + rel_direction(trial_coords(agent), trial_coords(agent.selected_sheep), trial_coords(target)))/2;
enddefine;
...
define :method forward(agent:trial_agent, speed);
    ...
    for other in all_agents do
        ...
        if other /== agent then
            ...
            trial_size(agent) + trial_size(other) + 2 -> safe_range;
            ...
            while sim_distance_from( aimx, aimy, trial_coords(other) ) <= safe_range do
                ;;; If so then re-adjust the agent's speed until the two
                ;;; do not collide.
...
define :method move_sheep(agent:trial_sheep, bearing, speed );
...
define :method wander(agent:trial_sheep);
    ...
    move_sheep( agent, (trial_heading(agent) + random(70) - 35) mod 360, 1 );
enddefine;
...
/*
PROCEDURE: is_in_pen (object, tolerance) -> boole
INPUTS   : object is a sheep agent, tolerance a tolerance
OUTPUTS  : boole, is a boolean
USED IN  : ???
CREATED  : 29 Jul 1999 TC
...
define is_in_pen(object, tolerance);
    ;;; changed 16 Nov 2003
    ;;; so that dog can use a different tolerance from sheep
    close_to_pen(object, tolerance);
enddefine;
...
define :method make_sheep_list(agent:trial_dog, objects)->list;
    ...
    lvars x;
    [%for x in objects do
         if istrial_sheep(x) and not(is_in_pen(x, 0)) then
           x
         endif;
    endfor%] -> list;
enddefine;
...
same_side_of_line(post1, post7, gromit, gromit.selected_sheep)
...
define :method in_front_of_pen(agent:trial_dog) -> boole;
    ...
    if sect(agent) < 5 then
        ...
        not(is_in_pen(agent.selected_sheep, -3)) and
            not(is_in_pen(agent, 5)) and
            (sect(agent) == 5) and
            same_side_of_line(lpost, hpost, agent, agent.selected_sheep)
    endif -> boole;
enddefine;
...
define :method behind_pen(agent:trial_dog) -> boole;
    ...
    sect(agent) == 5
    and not(is_in_pen(sheep, 0))
    and not(is_in_pen(agent, 5))
...
/*
;;; No longer needed as movable objects are already rc_selectable
...
define :ruleset dog_perception_rules;
    ...
    [WHERE sim_distance(sheep, targ) < 35
        and is_in_pen(sheep, -3) and
            close_to_pen(sim_myself, 30)] ;;; increased 16 Nov 2003
    ==>
    [POP11
        [] ->> selected_sheep(sim_myself)
...
define :ruleset dog_target_rules;
    ...
    [LVARS [sheep = selected_sheep(sim_myself)] ];
...
RULE sheeppensector
    ...
    ==>
    [LVARS [sector = pen_sector(sim_myself.selected_sheep)]]
    [POP11
...
RULE select_sheep
    [WHERE sim_myself.selected_sheep == []
        and
    ...
    [POP11
        ...
        sheep -> sim_myself.selected_sheep;
    ]
    [NOT current ==]
    [current ?sheep]
...
RULE counterhit
    ;;; rule for escaping from (simple) traps: move at random roughly backwards
    [counter 2]
        ==>
        ...
        [heading =
            round(
                (sim_myself.trial_heading
                    + if random(2) == 1 then 80 + random(40) else - 80 - random(40) endif)
                    + (5 - random(10)) ) mod 360]]
    [do move_dog 35 ?heading]
...
define :ruleset take;
    [DLOCAL [prb_allrules = false] ];
    [VARS [sheep = selected_sheep(sim_myself)]];
...
define :ruleset treedetection;
    ...
    [LVARS
        ...
        [sheep = selected_sheep(sim_myself)]
    ];
...
define :ruleset sheep_instinct_rules;
    ...
    [Urgency ?value]
    [WHERE not(is_in_pen(sim_myself, 0)
...
define :method rc_add_pic_to_window(pic:trial_obstacle, win_obj:rc_window_object, atfront);
    ...
    if rc_event_types(win_obj) == [] then
        ;;; make sure mere mere movements of mouse trigger no events
        rc_mousepic(win_obj, [button dragonly])
    endif;
...
/* --- Revision History ---------------------------------------------------
--- Aaron Sloman, 24 Nov 2003
    Introduced more uses of rc_in_event_handler to prevent interference
    between different graphical processes driven by the mouse and by
    the simulation
