/* --- The University of Birmingham 1995.  --------------------------------
 > File:            $poplocal/local/prb/test/prbriver2.p
 > Purpose:         A test file for poprulebase
 > Author:          Aaron Sloman, Jul  2 1995
 > Documentation:   TEACH PRBRIVER
 > Related Files:
                    $poplocal/local/prb/test/prbriver.p
 */
...
NB
    Unlike the teach file and the prbriver.p file this one uses
    the format [isat item place] instead of [item isat place]

    This requires several modifications to the code, including
    sorting on second item of the database instead of the first
*/
...
prb_run(prb_rules, [[goal [[isat man left] [isat grain right]] ]]);
prb_run(prb_rules, [[goal [[isat man right] [isat chicken left]] ]]);

prb_run(prb_rules,
    [[goal [[isat chicken left] [isat fox right][NOT isat man right]] ]]);

prb_run(prb_rules,
    [[goal [[isat chicken left] [isat grain right][NOT isat man left]] ]]);
...
define :rule complete_move in solve_rules
    ...
    [LVARS [newstate = thing_data()]]
    [MODIFY ?Cond2 state ?newstate]
