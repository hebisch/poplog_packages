...
;;; Now a version that instantiates before adding

define lconstant prb_ADDINSTANCE(rule_instance, action);
    prb_add(prb_instance(fast_back(action)));
enddefine;

prb_ADDINSTANCE -> prb_action_type("ADDINSTANCE");
...
define lconstant prb_TEST_ADDINSTANCE(rule_instance, action);
    lvars rest = prb_instance(fast_back(action));
    unless prb_instance_present(rest) then prb_add(rest) endunless
enddefine;

;;;prb_TEST_ADDINSTANCE -> prb_action_type("TEST_ADDINSTANCE");
prb_TEST_ADDINSTANCE -> prb_action_type("TESTADDINSTANCE");
...
define vars prb_run_with_matchvars(rules, database, limit);
    ...
    dlocal
        ...
        prb_rules,                  ;;; set to [] globally
        prb_ruleset_name,           ;;; set false globally
        prb_current_family,         ;;; set false globally
...
/* --- Revision History ---------------------------------------------------
--- Aaron Sloman, Nov 24 2011
        Changed prb_run_with_matchvars so that if prb_run is called with a
        word (other than "prb_rules") as its first argument, then at the start
        of the main loop it gets the recursive_valof the word and uses that as
        prb_rules from then on. This allows the ruleset given to prb_run to be
        edited while the program is waiting for input. Useful for debugging.
--- Aaron Sloman, Oct 22 2011
        Added new action types ADDINSTANCE, TESTADDINSTANCE
    Suggested by Brian Logan. For use when storing an instance of a pattern,
    e.g. a pattern that comes from an ALL condition being satisfied.
