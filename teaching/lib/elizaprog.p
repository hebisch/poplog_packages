...
;;; Make sure items in $usepop/pop/packages/teaching are accessible
uses teaching;
...
define changeperson(sentence) -> sentence;
    dlocal sentence;        ;;; NB sentence is used non-locally
...
define readsentence() -> sentence;
    ...
    for list on sentence do
        front(list) -> word;
        if isword(word) and strmember(last(word), '.,;:!?') then
    ...
    ;;; handle special 'executive' instructions
...
define firstolast(list) -> (first,list);
    ...
    ;;; where appropriate
    ;;; next bit replaced in web eliza
...
define macro CIRCULATE;
    ...
    ;;;   Changed to use shuffle in web eliza
...
define :newrule money;
    if ithasoneof([money cash broke grant rent pay job cost grant budget
        salary tax government holiday expensive price brown
        fees afford
        rich wealthy poor cash mortgage credit debit pay pound dollar yen euro cent penny]) then
        ...
        CIRCULATE
            [
                ...
                'Do you donate to good causes, e.g. a fund for lonely robots?'
...
/*
;;; old version, replaced by version in online eliza
define itsaquestion_old;
    ...
    if itsaquestion() then
        ...
        and random(100) < 20 then
            ...
            [
                ...
                [Nowhere did ?L1 ??L2 -- as far as I know]
            ]
...
define intentionword(word);
    member(word,
        [hope expect wish want intend aim desire prefer like value])
enddefine;
    ...
    then
        CIRCULATE
          [
            ...
            ['Why do you' ?L1 to ??L2]
            ['When do you think machines will have desires or intentions?']
    ...
    then
        CIRCULATE
          [
            ...
            ['Perhaps the next robot is also going to' ??L2]
...
define :newrule suppnot;
    ...
    elseif itslikeoneof([[??L1:nonnull is not ??L2:nonnull]
                                    [??L1:nonnull are not ??L2:nonnull]])
    then
        CIRCULATE
          [[suppose ??L1 were ??L2]
            [is ??L1 really no ??L2?]
    ...
    elseif itslikeoneof( [[??L1:nonnull am ??L2]] ) then
        ...
        [
            ...
            ['well educated people know' ??L1 'am not' ??L2]
...
'The inadequacies of human thought is shown in the things humans do'
'Are humans intelligent enough to design intelligent machines?'
'what do you really think of computers?'
'Was Steve Jobs a great computer scientist?'
] -> complist;
...
define :newrule computer;
    if ithasoneof([micro eliza vax mac sparc program computer
        ...
        linux solaris unix vista win7 xp osx]) then
    ...
    if  not(itsaquestion()) then
        ...
        then
            ...
            [
                ...
                ['Is David Lodge, the novellist, conscious' ??L2 ?]
                ...
                'Is quantum entanglement a requirement for consciousness?'
                ['Could a machine be conscious' ??L2]
            ]
        elseif itslikeoneof([
                    [i am aware ??L2]
                    [i am conscious ??L2]
        ...
        then
            ...
            [
                ...
                'Do you think David Lodge is conscious?'
...
define :newrule fantasy;
    ...
    elseif not(itslikeoneof([[you do ==] [i do == ] [you do not ==] [i do not ==] [you have ==] [i have ==]]))
...
;;; NB The Web version is different.
...
define transform_output(response) -> response;
    ...
    while response matches [??L1 me will ??L2] do
        [^^L1 ^^(oneof([[this machine][eliza] [yours truly]])) will ^^L2] -> response;
    endwhile;
...
/*  --- Revision History ---------------------------------------------------
--- Aaron Sloman, Nov 13 2011
        Handled me will in output
--- Aaron Sloman, Oct 19 2011
        Fixed bug triggered by number in input sentence.
        Added more options to lists of canned strings.
