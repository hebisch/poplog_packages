/* --- Copyright University of Sussex 1991. All rights reserved. ----------
 > File:            C.all/x/pop/lib/xpm_listutils.p
...
lconstant
    ScrollingList = XptWidgetSet("Motif")("ListWidget"),
...
define lconstant View_item(w, item);
    lvars w, item, top, count;
    fast_XptValue(w, XtN topItemPosition) -> top;
    fast_XptValue(w, XtN visibleItemCount) -> count;
...
Makes a specific item the current list item
*/

define lconstant Set_current_item(widget, items, item);
    lvars widget, item, items, old_item;
    XmListDeselectAllItems(widget);
    XmListSelectItem(widget, item, false);
    View_item(widget, item);
enddefine;
...
define lconstant Get_selected_items(widget, current_items, wants_tokens);
    ...
    returnunless(current_items)(#_<identfn(%termin%)>_#);
    lconstant ptr = EXPTRINITSTR(:exptr),
            count = writeable initintvec(1);
    unless XmListGetSelectedPos(widget, ptr, count) then
        return(#_<identfn(%termin%)>_#);
    endunless;
    exacc :exptr ptr -> items; count(1) -> num;
...
/* Check_widget(w) -
Checks for subclasses of MOTIF ScrollingList
*/

define lconstant Check_widget(widget);
    lvars widget;
    unless fast_XtIsSubclass(XptCheckWidget(widget), ScrollingList) then
        mishap(widget,1,'Motif ListWidget Needed');
    endunless;
enddefine;

define global XpmSelectedListItems(widget) -> repeater;
...
define updaterof XpmSelectedListItems(repeater, widget);
    ...
    Check_widget(widget);
    fast_XptValue(widget, XtN selectionPolicy, "byte" ) == XmSINGLE_SELECT
            -> one_only;
    ...
    fast_repeat;
        ...
        unless token.islist_item then
            Get_item_token(widget, current_items, token) -> token;
            unless token then mishap(item,1,'ITEM NOT IN LIST') endunless;
        endunless;
        XmListSelectItem(widget, token, false);
        quitif(one_only);
    endfast_repeat;
enddefine;

define global XpmListItems(widget) -> repeater;
    lvars widget, repeater, wants_tokens = false, items;
    if widget.isboolean then widget -> (widget, wants_tokens) endif;
    Check_widget(widget);
    list_item_assoc(widget) -> items;
...
define global XpmListTokenToItem = Get_token_item enddefine;

define global XpmListItemToToken(widget, item);
    lvars widget, item;
    Check_widget(widget);
...
define global XpmCurrentListItem(widget);
...
define updaterof XpmCurrentListItem(item, widget);
    ...
    unless item then XmListDeselectAllItems(widget); return endunless;
...
define global XpmSubscrListItems(n, widget) -> item;
...
define global XpmNumListItems(widget);
...
define global XpmViewListItem(widget, item);
...
global constant xpm_listutils = true; /* for uses */
